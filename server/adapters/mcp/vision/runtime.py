...
def build_vision_runtime_config(config: Config) -> VisionRuntimeConfig:
    ...
    return VisionRuntimeConfig(
        ...
        capture_grid_enabled=bool(getattr(config, "VISION_CAPTURE_GRID_ENABLED", False)),
        transmit_auxiliary_channels=bool(getattr(config, "VISION_TRANSMIT_AUX_CHANNELS", False)),
        mark_overlay_enabled=bool(getattr(config, "VISION_MARK_OVERLAY_ENABLED", False)),
