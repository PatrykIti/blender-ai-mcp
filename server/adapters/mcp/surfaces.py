...
SURFACE_PROFILES: dict[str, SurfaceProfileSettings] = {
    ...
    "llm-guided": SurfaceProfileSettings(
        ...
        instructions=(
            ...
            "If router_set_goal returns guided_handoff, treat it as typed continuation context and start from guided_handoff.direct_tools only while those tools remain directly visible. "
            "When visibility narrows later, trust live visibility_rules plus required_checks instead of retrying stale names through call_tool(...). "
            ...
            "Treat the live guided flow as the primary execution contract. If current_step is checkpoint_iterate, do not keep creating new semantic parts by default; run the staged compare loop or the explicitly visible support tools first unless the same state still reports missing roles for the active workset. "
            "Use guided_role only when that role is currently exposed in guided_flow_state.allowed_roles. "
            "reference_compare_stage_checkpoint and reference_iterate_stage_checkpoint use checkpoint_label as the public label field; do not send legacy label or notes fields. "
            "macro_align_part_with_contact uses reference_object, target_relation, gap, align_mode, and optional normal_axis with only X, Y, or Z; do not invent contact_axis/contact_side or signed axes such as -Y. "
