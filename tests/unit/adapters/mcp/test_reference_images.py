...
def test_refresh_reference_understanding_summary_merges_optional_support_evidence(tmp_path, monkeypatch):
    ...
    class Backend:
        async def analyze(self, request):
            return {
                "status": "available",
                "understanding_id": "understanding_optional_support",
                "goal": request.goal,
                "reference_ids": ["ref_front"],
                "subject": {
                    "label": "low poly squirrel",
    ...
    responses = {
        "http://localhost:9200/classify": {
            "classification_scores": [
                ...
                {"label": "creature_blockout", "score": 0.41},
            ]
        },
            ...
            "segmentation_artifacts": [
                {
                    ...
                    "artifact_kind": "mask",
                    "reference_id": "ref_front",
                    "summary": "Support-only tail mask from the optional sidecar.",
                }
            ]
        },
    }
    ...
    assert any(
        item["source"] == "part_segmentation" for item in updated.reference_understanding_summary["source_provenance"]
    )
    assert any(
        item["evidence_id"] == "mask_tail_front"
        for item in updated.reference_understanding_summary["visual_evidence_refs"]
    )
    assert (
        captured[0]["json"]["reference_understanding"]["construction_strategy"]["construction_path"] == "low_poly_facet"
    )


def test_refresh_reference_understanding_summary_sanitizes_optional_support_payloads(tmp_path, monkeypatch):
    ctx = FakeContext()
    reference_path = tmp_path / "front.png"
    _write_test_silhouette(reference_path, with_ears=True)
    set_session_capability_state(
    ...
    class Backend:
        async def analyze(self, request):
            return {
                "status": "available",
                "understanding_id": "understanding_support_hygiene",
                "goal": request.goal,
            ...
            "segmentation_artifacts": [
                {
                    ...
                    "artifact_kind": "mask",
                    "reference_id": "ref_front",
                    "summary": "Support-only mask stored at /tmp/private/mask_tail_front.png",
                }
            ]
        },
    }
    ...
    assert updated.reference_understanding_summary["segmentation_artifacts"] == [
        {
            ...
            "artifact_kind": "mask",
            "reference_id": "ref_front",
            "summary": "Support-only mask stored at [redacted-path]",
        }
    ]
    assert all(
        "/tmp/private/" not in str(item.get("summary", ""))
        for item in updated.reference_understanding_summary["visual_evidence_refs"]
    )


def test_refresh_reference_understanding_summary_keeps_optional_support_failures_non_fatal(tmp_path, monkeypatch):
    ctx = FakeContext()
    reference_path = tmp_path / "front.png"
    _write_test_silhouette(reference_path, with_ears=True)
    set_session_capability_state(
    ...
    class Backend:
        async def analyze(self, request):
            return {
                "status": "available",
                "understanding_id": "understanding_optional_support_failure",
                "goal": request.goal,
                "reference_ids": ["ref_front"],
                "subject": {
                    "label": "low poly squirrel",
    ...
    responses = {
        "http://localhost:9200/classify": RuntimeError("classifier down"),
        "http://localhost:9100/segment": RuntimeError("segmentation down"),
    }
    ...
    assert updated.reference_understanding_summary is not None
    assert updated.reference_understanding_summary["status"] == "available"
    assert updated.reference_understanding_summary["classification_scores"] == []
    assert updated.reference_understanding_summary["segmentation_artifacts"] == []
    assert updated.reference_strategy_state is not None
    assert updated.reference_strategy_state["primary_family"] == "modeling_mesh"
    assert any(
        item["source"] == "classification_scores" and "unavailable" in str(item.get("summary", ""))
        for item in updated.reference_understanding_summary["source_provenance"]
    )
    assert any(
...
def test_refresh_reference_understanding_summary_rebuilds_missing_strategy_state_without_rerunning_backend(
    tmp_path,
    monkeypatch,
):
    ctx = FakeContext()
    ...
    set_session_capability_state(
        ...
        SessionCapabilityState(
            ...
            reference_understanding_summary={
                "status": "available",
                "understanding_id": "understanding_strategy_rebuild",
                "goal": "create a low-poly squirrel",
                "reference_ids": ["ref_front"],
                "subject": {
                    "label": "low poly squirrel",
    ...
    class Backend:
        async def analyze(self, request):
            raise AssertionError("refresh should rebuild missing strategy state without rerunning RU")
...
def test_refresh_reference_understanding_summary_merges_optional_support_on_cached_summary(tmp_path, monkeypatch):
    ctx = FakeContext()
    reference_path = tmp_path / "front.png"
    _write_test_silhouette(reference_path, with_ears=True)
    set_session_capability_state(
        ...
        SessionCapabilityState(
            ...
            reference_understanding_summary={
                "status": "available",
                "understanding_id": "understanding_cached_support_refresh",
                "goal": "create a low-poly squirrel",
                "reference_ids": ["ref_front"],
                "subject": {
                    "label": "low poly squirrel",
    ...
    class Backend:
        async def analyze(self, request):
            raise AssertionError("cached summary refresh should not rerun base reference-understanding backend")
...
def test_refresh_reference_understanding_summary_retries_optional_support_after_failure_provenance(
    tmp_path,
    monkeypatch,
):
    ctx = FakeContext()
    ...
    set_session_capability_state(
        ...
        SessionCapabilityState(
            ...
            reference_understanding_summary={
                "status": "available",
                "understanding_id": "understanding_retry_support_refresh",
                "goal": "create a low-poly squirrel",
                "reference_ids": ["ref_front"],
                "subject": {
                    "label": "low poly squirrel",
                ...
                "source_provenance": [
                    {"source": "reference_understanding"},
                    {
                        ...
                        "summary": "Optional reference classifier unavailable: timeout",
                    },
                ],
                "boundary_policy": {
                    "advisory_only": True,
    ...
    class Backend:
        async def analyze(self, request):
            raise AssertionError("cached support retry should not rerun base reference-understanding backend")
    ...
    assert any(
        item["source"] == "classification_scores" and "unavailable" not in str(item.get("summary", "")).lower()
        for item in updated.reference_understanding_summary["source_provenance"]
    )


def test_refresh_reference_understanding_summary_rebuilds_missing_gate_ids_from_gate_plan(tmp_path, monkeypatch):
    reference_path = tmp_path / "front.png"
    _write_test_silhouette(reference_path, with_ears=True)
    ctx = FakeContext()
    session = SessionCapabilityState(
        ...
        reference_understanding_summary={
            "status": "available",
            "understanding_id": "understanding_gate_ids_rebuild",
            "goal": "create a low-poly squirrel",
            "reference_ids": ["ref_front"],
            "subject": {
                "label": "low poly squirrel",
    ...
    class Backend:
        async def analyze(self, request):
            raise AssertionError("gate id rebuild should not rerun reference-understanding backend")
...
def test_refresh_reference_understanding_summary_keeps_visual_evidence_refs_in_sync_with_segmentation_limit(
    tmp_path,
    monkeypatch,
):
    ctx = FakeContext()
    ...
    class Backend:
        async def analyze(self, request):
            return {
                "status": "available",
                "understanding_id": "understanding_segmentation_limit",
                "goal": request.goal,
                "reference_ids": list(request.metadata.get("reference_ids") or []),
                "subject": {
                    "label": "low poly squirrel",
    ...
    artifacts = [
        {
            "artifact_id": f"mask_{index}",
            "artifact_kind": "mask",
            "reference_id": "ref_front",
            "summary": f"Support-only mask {index}.",
        }
        for index in range(1, 21)
    ]
    responses = {"http://localhost:9100/segment": {"segmentation_artifacts": artifacts}}
    ...
    assert updated.reference_understanding_summary is not None
    assert len(updated.reference_understanding_summary["segmentation_artifacts"]) == 20
    segmentation_evidence_refs = [
        item
        for item in updated.reference_understanding_summary["visual_evidence_refs"]
        if item["source_class"] == "part_cue"
    ]
...
def test_refresh_reference_understanding_summary_does_not_retry_support_after_empty_success(
    tmp_path,
    monkeypatch,
):
    ctx = FakeContext()
    ...
    set_session_capability_state(
        ...
        SessionCapabilityState(
            ...
            reference_understanding_summary={
                "status": "available",
                "understanding_id": "understanding_empty_support_retry",
                "goal": "create a low-poly squirrel",
                "reference_ids": ["ref_front"],
                "subject": {
                    "label": "low poly squirrel",
    ...
    class Backend:
        async def analyze(self, request):
            raise AssertionError("same-goal empty support retry should not rerun base reference-understanding backend")
    ...
    captured: list[dict[str, Any]] = []
    responses = {
        "http://localhost:9200/classify": {"classification_scores": []},
        "http://localhost:9100/segment": {"segmentation_artifacts": []},
    }
    ...
    captured.clear()
    updated_again = asyncio.run(refresh_reference_understanding_summary_async(ctx, session=updated))
    assert updated_again.reference_understanding_summary is not None
    assert updated_again.reference_understanding_summary["classification_scores"] == []
    assert updated_again.reference_understanding_summary["segmentation_artifacts"] == []
    assert captured == []
    ...
    set_session_capability_state(
        ...
        SessionCapabilityState(
            ...
            reference_understanding_summary={
                ...
                "segmentation_artifacts": [
                    {
                        "artifact_id": "mask_seat_front",
                        "artifact_kind": "mask",
                        "reference_id": "ref_active",
                        "summary": "Support-only seat mask for front-view follow-up.",
                    }
                ],
                "verification_requirements": [],
                "source_provenance": [{"source": "reference_understanding"}],
...
def test_reference_compare_stage_checkpoint_keeps_super_complex_packets_within_runtime_image_budget(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("BLENDER_AI_TMP_INTERNAL_DIR", str(tmp_path / "internal"))
    monkeypatch.setenv("BLENDER_AI_TMP_EXTERNAL_DIR", str(tmp_path / "external"))
    ...
    result = asyncio.run(
        reference_compare_stage_checkpoint(
            ctx,
            target_object="Creature",
            checkpoint_label="stage_super_complex_budget",
            preset_profile="rich",
            target_view="front",
        )
    )
    ...
    assert all(
        [image.label for image in request.images if image.role == "reference"] == [f"front_ref_{index}"]
        for index, request in enumerate(captured_requests, start=1)
    )
...
def test_reference_compare_stage_checkpoint_marks_one_image_budget_as_uncertain_without_runner_call(
    ...
    async def _unexpected_run_vision_assist(ctx, *, request, resolver):
        raise AssertionError("vision_assist must not run without a packet-local reference slice")
    ...
    result = asyncio.run(
        reference_compare_stage_checkpoint(
            ctx,
            target_object="Creature",
            checkpoint_label="stage_one_image_budget",
            preset_profile="rich",
            target_view="front",
        )
    )
    ...
    assert result.error is None
    assert result.compare_diagnostics is not None
    packet = result.compare_diagnostics.packets[0]
    assert packet.capture_labels == ["target_front_after"]
    assert packet.reference_ids == []
    assert packet.extraction_status == "blocked"
    assert packet.ranking_status == "skipped"
    ...
    assert any("blocked, low-information, or failed" in note for note in result.compare_diagnostics.conflict_notes)
    assert result.reference_orchestrator_feedback is not None
    assert any(
        "Runtime image budget is below the 2-image minimum" in note
...
def test_reference_compare_stage_checkpoint_projects_compare_time_segmentation_sidecar_into_packets(
    tmp_path, monkeypatch
):
    reference_path = tmp_path / "reference_front.png"
    capture_path = tmp_path / "capture_front.png"
...
def test_reference_compare_stage_checkpoint_keeps_available_segmentation_sidecar_advisory_only(tmp_path, monkeypatch):
    reference_path = tmp_path / "reference_front.png"
    capture_path = tmp_path / "capture_front.png"
    _write_upper_profile_silhouette(reference_path, upper_width=80)
    _write_upper_profile_silhouette(capture_path, upper_width=80)
    monkeypatch.setenv("BLENDER_AI_TMP_INTERNAL_DIR", str(tmp_path / "internal"))
    monkeypatch.setenv("BLENDER_AI_TMP_EXTERNAL_DIR", str(tmp_path / "external"))
    ...
    async def _fake_run_vision_assist(ctx, *, request, resolver):
        ...
        return AssistantRunResult(
            ...
            result=VisionAssistContract(
                backend_kind="mlx_local",
                model_name="mlx-community/Qwen3-VL-4B-Instruct-4bit",
                goal_summary="Front packet is clean.",
                reference_match_summary="The packet has no actionable mismatch.",
                visible_changes=["Front silhouette is readable."],
                shape_mismatches=[],
                proportion_mismatches=[],
                correction_focus=[],
...
def test_reference_compare_stage_checkpoint_maps_packet_guidance_into_compare_diagnostics(tmp_path, monkeypatch):
    ...
    async def _fake_run_vision_assist(ctx, *, request, resolver):
        return AssistantRunResult(
            ...
            result=VisionAssistContract(
                backend_kind="mlx_local",
                model_name="mlx-community/Qwen3-VL-4B-Instruct-4bit",
                goal_summary="Front packet looks clean relative to the reference.",
    ...
    result = asyncio.run(
        reference_compare_stage_checkpoint(
            ctx,
            target_object="Squirrel",
            checkpoint_label="stage_front_clean",
            preset_profile="rich",
        )
    )
    ...
    assert result.compare_diagnostics is not None
    assert result.compare_diagnostics.packet_count == 1
    assert result.compare_diagnostics.packets[0].extraction_status == "success"
    assert result.compare_diagnostics.packets[0].ranking_status == "not_needed"
...
def test_reference_compare_stage_checkpoint_emits_compact_diagnostics_for_packet_evidence_refs(tmp_path, monkeypatch):
    ...
    async def _fake_run_vision_assist(ctx, *, request, resolver):
        ...
        if compare_phase == "packet_extraction":
            return AssistantRunResult(
                ...
                result=VisionAssistContract(
                    backend_kind="mlx_local",
                    model_name="mlx-community/Qwen3-VL-4B-Instruct-4bit",
                    goal_summary="Front packet has enough signal for one correction.",
                    visible_changes=["Front silhouette is readable."],
                    shape_mismatches=["Head silhouette is still too round."],
                    correction_focus=["Head silhouette"],
                    packet_guidance=VisionPacketStatusContract(
                        packet_status="ready",
                        status_reason=None,
        ...
        return AssistantRunResult(
            ...
            result=VisionAssistContract(
                backend_kind="mlx_local",
                model_name="mlx-community/Qwen3-VL-4B-Instruct-4bit",
                goal_summary="Head silhouette is still too round.",
                visible_changes=["Front silhouette is readable."],
                shape_mismatches=["Head silhouette is still too round."],
                correction_focus=["Head silhouette"],
    ...
    result = asyncio.run(
        reference_compare_stage_checkpoint(
            ctx,
            target_object="Squirrel",
            checkpoint_label="stage_front_actionable_compact",
            preset_profile="compact",
        )
    )
...
def test_reference_compare_stage_checkpoint_preserves_extraction_when_packet_ranking_fails(tmp_path, monkeypatch):
    ...
    result = asyncio.run(
        reference_compare_stage_checkpoint(
            ctx,
            target_object="Squirrel",
            checkpoint_label="stage_front_rank_fail",
            preset_profile="rich",
        )
    )

    assert result.error is None
    assert [request.metadata["compare_phase"] for request in requests] == ["packet_extraction", "packet_ranking"]
    packet_id = requests[0].metadata["packet_id"]
    assert requests[1].metadata["packet_id"] == packet_id
    assert f"packet_id={packet_id}" in requests[0].prompt_hint
    assert f"packet_id={packet_id}" in requests[1].prompt_hint
    assert result.vision_assistant is not None
    assert result.vision_assistant.status == "success"
    assert result.vision_assistant.result is not None
    assert result.vision_assistant.result.shape_mismatches == ["Head silhouette is still too spherical."]
    assert result.compare_diagnostics is not None
    assert result.compare_diagnostics.packet_order == [packet_id]
    assert result.compare_diagnostics.packets[0].packet_id == packet_id
    assert result.compare_diagnostics.packets[0].extraction_status == "success"
...
def test_reference_compare_stage_checkpoint_emits_compare_diagnostics_on_compact_ranking_downgrade(
    tmp_path, monkeypatch
):
    image_front = tmp_path / "front.png"
    image_front.write_bytes(b"front")
        ...
        if compare_phase == "packet_extraction":
            return AssistantRunResult(
                ...
                result=VisionAssistContract(
                    backend_kind="mlx_local",
                    model_name="mlx-community/Qwen3-VL-4B-Instruct-4bit",
                    goal_summary="Front packet has enough signal for ranking.",
                    visible_changes=["Front silhouette is visible."],
                    shape_mismatches=["Possible head silhouette issue."],
                    correction_focus=["Head silhouette"],
                    packet_guidance=VisionPacketStatusContract(
                        packet_status="ready",
                        status_reason=None,
    ...
    result = asyncio.run(
        reference_compare_stage_checkpoint(
            ctx,
            target_object="Squirrel",
            checkpoint_label="stage_front_rank_downgrade_compact",
            preset_profile="compact",
        )
    )
    ...
    assert result.compare_diagnostics is not None
    packet = result.compare_diagnostics.packets[0]
    assert packet.extraction_status == "success"
