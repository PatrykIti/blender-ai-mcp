...
def test_reference_compare_stage_checkpoint_rich_profile_exposes_ready_sculpt_planner_detail(
    ...
    try:
        ...
        monkeypatch.setattr("server.adapters.mcp.areas.reference.run_vision_assist", _fake_run_vision_assist)
        # Keep this case focused on the rich-profile planner handoff with deterministic clean view evidence
        # while still exercising the structured diagnostics -> hint builder path.
        monkeypatch.setattr(
            scene_handler,
