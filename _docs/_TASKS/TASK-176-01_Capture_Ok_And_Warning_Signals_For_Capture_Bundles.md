...
**Acceptance Criteria:**
- a camera/view operation (`isolate_object`, `set_standard_view`, `camera_focus`, `camera_orbit`) that returns a string matching a bounded set of known failure markers (e.g. a normalized `"no 3d viewport found"` substring sourced from the addon handlers) is treated as a failed step for that preset; success-confirmation strings are NOT treated as failure
