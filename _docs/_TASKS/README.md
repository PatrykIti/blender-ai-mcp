...
Implementation status (2026-05-30/31 audit-fix branch; validation tracked by changelogs 389-392). **Fully done families in the `TASK-174` through `TASK-183` Vision Output Quality wave: `TASK-174`, `TASK-175`, `TASK-176`, `TASK-178`, and `TASK-183`.** The audit-fix branch closes previously identified drift in field descriptions/read-order docs, packet-synthesis truncation accounting, top-level confidence clamping, optional grid transmission, live capture-side object-ID IoU sidecars, default-off auxiliary depth/normal/object-ID transmission, default-off live Set-of-Mark overlay capture, capability-aware schema/payload routing, provider usage reporting, and lightweight silhouette consistency/convergence. `TASK-177`, `TASK-179`, `TASK-180`, `TASK-181`, and `TASK-182` remain open for the explicit follow-ons below.

Promoted open child work after the audit refresh:
- **server-side / preset orchestration:** `TASK-177-02` (orthographic top plus oblique 3/4 compact-preset default), `TASK-179-02` (fixture-calibrated per-object IoU thresholds and closeout after live sidecar population and auxiliary-channel transmission)
- **Set-of-Mark live path:** `TASK-180-02` (stable cross-view mark ids), `TASK-180-03` (default-off reference-image marks sidecar), `TASK-180-04` (live mark-keyed compare payload, typed correspondence table, and bounded validity retry)
- **graph/loop authority:** `TASK-181-01` (registry-as-compare-scope staged-compare rewrite), `TASK-182-01` (full Critic/Verify rerender round-trip), `TASK-182-02` (deterministic-first layered exit; channel consolidation substrate already shipped)

Completed substrates and child slices that remain intentionally nested under open umbrellas: `TASK-179-01`, `TASK-179-03`, the helper/live-sidecar portions of `TASK-179-02`, `TASK-180-01`, `TASK-181-02`, `TASK-181-03`, `TASK-181-04`, and the server-side helper portions of `TASK-180-04`. These are not counted as full umbrella completion until their parent integration lanes are closed.
