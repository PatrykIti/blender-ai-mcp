...
Implementation status (2026-05-30, validated against the full Blender E2E suite; full `tests/unit` green at 3575). **Fully done (7 families): `TASK-174`, `TASK-175`, `TASK-176`, `TASK-177`, `TASK-178`, `TASK-182`-consolidation lands but `TASK-182` umbrella stays open for its layered-exit half, `TASK-183`.** Specifically every subtask of 174/175/176/177/178/183 is complete (changelogs 368–386), plus `TASK-180-01`/`TASK-180-04`, `TASK-181-02`/`TASK-181-03`/`TASK-181-04`, and `TASK-182-01`/`TASK-182-02`-consolidation. Only **5 subtasks remain open**, all requiring new in-Blender code, a heavy optional model, or a deep guided-flow rewrite validated through the ~18-minute `run_e2e_tests.py` cycle: `TASK-179-01` (addon `bpy` object-ID/Z-depth/normal compositor render passes + RPC), `TASK-179-03` (auxiliary-channel transmission, depends on 179-01), `TASK-180-02` (stable cross-view mark ids keyed to the live part registry — needs the 179-01 object-ID pass), `TASK-180-03` (reference-image marks via the default-off Grounded-SAM sidecar — needs the optional heavy model installed), and `TASK-181-01` (registry-as-compare-scope, the deep guided-flow rewrite that fixes the `TASK-173` Body+Head drift). The `TASK-182` layered-exit half (verifier authority + Critic/Verify re-render) also remains. Two live local-MLX model-quality E2E tests fail only under full-suite execution (MLX memory/ordering variance) and pass standalone; they do not exercise the changed capture path.

Exactly which subtasks remain open (10):
- **server-side, no addon, no E2E ripple:** `TASK-181-02` (typed graph-vs-graph diff contract), `TASK-180-04` (mark-keyed findings parser + correspondence table).
- **server-side but ripples into E2E preset/orchestration tests:** `TASK-177-02` (oblique 3/4 view in the compact preset — changes orbit/preset-count assertions in two E2E files), `TASK-177-03` (optional labelled grid composite), `TASK-182-02` (deterministic-first layered exit + channel consolidation — staged-compare orchestration), `TASK-181-04` (squirrel scope-drift regression lane).
- **needs new addon `bpy` code + RPC + the ~18-min E2E cycle:** `TASK-179-01` (object-ID/Z-depth/normal compositor passes), `TASK-179-03` (auxiliary-channel transmission, depends on 179-01), `TASK-180-02` (stable cross-view mark ids keyed to the live part registry), `TASK-180-03` (reference-image marks via the default-off Grounded-SAM sidecar — needs the optional heavy model installed), `TASK-181-01` (registry-as-compare-scope, the deep guided-flow rewrite that fixes the `TASK-173` Body+Head drift).

| ID | Title | Priority | Notes |
|----|-------|----------|-------|
| [TASK-173](./TASK-173_Reference_Guided_Creature_Scope_Convergence_And_Optional_Grounding_Followups.md) | **Reference-Guided Creature Scope Convergence And Optional Grounding Follow-Ups** | 🔴 High | Follow-on after `TASK-135`, `TASK-169`, `TASK-171`, and `TASK-172` from a real squirrel session where classifier and vision answered, but the loop still converged only to primitive blockout because compare scope drifted toward `Body + Head`, whole-creature convergence lost precedence, and optional grounding stayed dormant where appendage ambiguity remained unresolved. |
...
| [TASK-177](./TASK-177_Reachable_Rich_Multi_View_Capture_And_Top_View_Default.md) | **Reachable Rich Multi-View Capture And Top-View Default** | 🟡 Medium | Quick win (M). The rich 8-view preset is unreachable under default config (`policy.py:20` vs default `max_images=8`), so only the compact triad ships. Decouple capture from transmission, add an orthographic top view (VSI-Bench / DiffuRank / IG-VLM). |
...
| [TASK-179](./TASK-179_Blender_Depth_Normal_And_Object_ID_Auxiliary_Passes.md) | **Blender Depth/Normal/Object-ID Auxiliary Passes** | 🔴 High | Structural (L). All deterministic evidence is single-view 2D silhouette, blind to depth/volume, though Blender emits depth/normal/object-ID passes ~free. Add auxiliary channels + per-object mask IoU without SAM (SpatialRGPT / SD-VLM). Enables `TASK-180`. |
| [TASK-180](./TASK-180_Set_Of_Mark_Object_Bound_Visual_Marks.md) | **Set-of-Mark Object-Bound Visual Marks** | 🔴 High | Advanced (XL). No per-object binding, so similar parts (which of four legs?) cannot be disambiguated. Overlay deterministic numbered marks keyed to registered parts on render and reference (Set-of-Mark / Grounded SAM / SAM2 / BLINK). Depends on `TASK-179`. |
| [TASK-181](./TASK-181_Scene_Graph_Diff_Compare_And_Registry_Scoped_Convergence.md) | **Scene-Graph Diff Compare And Registry-Scoped Convergence** | 🔴 High | Advanced (XL). Compare has no parts inventory or expected relations and scope is ad-hoc — the root of the `TASK-173` squirrel scope drift. Make the part registry the canonical compare scope and emit a graph-vs-graph diff as relation triplets, not raw coordinates (SceneVerse / 3DGraphLLM / Text-Scene). |
| [TASK-182](./TASK-182_Critic_Verify_Compare_Loop_And_Deterministic_Exit_Criteria.md) | **Critic/Verify Compare Loop And Deterministic Exit Criteria** | 🔴 High | Structural (L). Nothing re-renders after an edit to verify each defect was resolved, and 4-6 overlapping correction channels have unclear precedence. Split Critic/Verify with a deterministic-first layered exit (LL3M / IR3D-Bench / SayPlan). |
