...
Implementation status (2026-05-30, validated against the full Blender E2E suite). Every family has at least one shipped, tested, committed slice; the full `tests/unit` suite (3549) is green and the live-model E2E lane passes. Fully done: ✅ `TASK-174` (caption interleaving; a roster-format regression it introduced was caught by the live-model E2E lane and fixed, changelogs 368/377), ✅ `TASK-175`, ✅ `TASK-176` (changelogs 369–370), ✅ `TASK-178` (structured per-finding compare schema end-to-end: contract + strict schema/parser + defect ids + reporting propagation, changelogs 372/373/379). In progress with a shipped core slice: 🚧 `TASK-177` (177-01 budget-aware view selection, changelog 371), 🚧 `TASK-179` (179-02 deterministic per-object mask IoU, changelog 376), 🚧 `TASK-180` (180-01 deterministic Set-of-Mark overlay + isolated-render orchestration, changelog 378), 🚧 `TASK-181` (181-03 relation-triplet prompt serialization, changelog 375), 🚧 `TASK-182` (182-01 stable defect-id substrate, changelog 373), 🚧 `TASK-183` (183-01 capability-aware schema + curated payload + a real findings/truncation backend-passthrough fix, changelog 374). Remaining subtasks are the addon `bpy` compositor/GPU render passes (179-01/03, 180 live wiring), deep packet-orchestration (181-01/02/04, 182 full Critic/Verify re-render loop), the optional grid composite (177-03), and silhouette calibration (183-02). They need new addon render-pass/overlay code and orchestration iterated through the ~18-minute `run_e2e_tests.py` cycle (and the default-off segmentation sidecar for reference parts), so they warrant dedicated E2E-validated sessions.
