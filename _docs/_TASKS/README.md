...
| [TASK-128-01](./TASK-128-01_Guided_Creature_Prompting_Handoff_And_Discovery_Hints.md) | **Guided Creature Prompting, Handoff, and Discovery Hints** | 🔴 High | Active Slice A. The platform baseline exists, but the creature prompt asset, goal-aware recommendation path, creature handoff narrowing, and discovery/search bias still need to be aligned with runtime behavior. The highest-risk open gap is now the session-aware creature handoff path: `guided_handoff` payload, session visibility/search shaping, and the matching regression harness still need to describe the same bounded low-poly creature story. |
