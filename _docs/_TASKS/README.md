...
| [TASK-166](./TASK-166_Hierarchical_Reference_Compare_Perceived_Evidence_And_Budget_Control.md) | **Hierarchical Reference Compare, Perceived Evidence, And Budget Control** | 🔴 High | In progress after the live packet-planning, packet-local narrow-question, synthesis, compare-time CV/segmentation, runtime-owned input budgets, and configured-vs-effective fail-safe budget diagnostics. Remaining open scope is the richer super-complex scheduling policy under `TASK-166-04-02` and the remaining public transparency hardening tracked under `TASK-166-06`. |
