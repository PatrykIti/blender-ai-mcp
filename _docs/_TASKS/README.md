...
| [TASK-141](./TASK-141_Guided_Creature_Run_Contract_And_Schema_Drift_Hardening.md) | **Guided Creature Run Contract and Schema Drift Hardening** | 🔴 High | Board-level follow-on from the first real TASK-128 squirrel run. Close the gap between documented/unit-tested guided contract rules and the actual active `llm-guided` surface used in real creature sessions, with end-to-end parity for discovery, `call_tool(...)`, reference intake, blockout-tool signatures, and `inspect_validate` handoff. Execution split now lives under `TASK-141-01` through `TASK-141-03`. |
