# TASK-113-05-01: Multiview Before/After Capture Contract

**Parent:** [TASK-113-05](./TASK-113-05_Vision_Measurement_And_Assertion_Layer.md)  
**Status:** ⏳ To Do  
**Priority:** 🔴 High

---

## Objective

Define the standard capture pattern for visual change analysis.

---

## Exact Documentation Targets

- `_docs/_MCP_SERVER/README.md`
- prompt docs for verification workflows
- any future vision-specific docs

---

## Required Contract

- before capture
- action/change
- after capture
- consistent view set
- structured diff summary or vision report

Possible standard views:

- front
- side
- top
- iso
- focus target view

---

## Acceptance Criteria

- “snapshot before and after” becomes a documented platform contract, not an improvised habit
