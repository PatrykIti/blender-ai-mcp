...
The E2E loop caught three real bugs during development (render-byte reversibility check; EEVEE not exposing `IndexOB`; undefined `original_samples` in the restore block) — all fixed. Follow-on `TASK-179-03` now ships the default-off server-side VLM auxiliary-image transmission lane.
**Priority:** 🔴 High
