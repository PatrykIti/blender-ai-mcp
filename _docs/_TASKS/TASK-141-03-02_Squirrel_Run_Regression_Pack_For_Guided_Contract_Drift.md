...
handoff shapes with focused automated coverage.
...
regressions, the contract can drift back without any signal until another real
session fails.
...
- add targeted unit regressions for the selected compatibility and guidance
  policies
- add/refresh E2E-style handoff coverage where the squirrel path depends on it
...
- update the parent summary so the shipped regression pack explicitly names the
  squirrel-run drift shapes it protects
