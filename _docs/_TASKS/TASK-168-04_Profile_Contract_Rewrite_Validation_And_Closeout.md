...
- post-closeout drift passes on 2026-05-16 repaired remaining compact-boundary,
  ...
  - `guided_register_part(...)` invalid-role, naming-policy, and scene-object
    validation failures persist typed fail-closed status instead of surfacing
    raw exceptions
  - normal clean compact staged compare/iterate omits `truth_bundle`,
    `truth_followup`, `correction_candidates`, and rich `planner_detail` while
