...
- implement the slice behavior end-to-end across: `server/adapters/mcp/surfaces.py`, `server/adapters/mcp/settings.py`, `tests/unit/adapters/mcp/test_server_factory.py`, `_docs/_MCP_SERVER/README.md`
...
- touch `server/adapters/mcp/settings.py` with an explicit change note (or explicit no-change rationale)
- touch `tests/unit/adapters/mcp/test_server_factory.py` with an explicit change note (or explicit no-change rationale)
