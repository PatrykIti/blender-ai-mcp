...
| `server/adapters/mcp/session_capabilities.py` | `describe_guided_flow_feedback(...)` facade | current guided-flow feedback seam | bounded guided-flow delta formatting should reuse the live summary helper instead of duplicating it |
...
- reuse `describe_guided_flow_feedback(...)` for bounded transition summaries
  rather than inventing a second prose formatter for the same state deltas
