...
| `openai_compatible_external` via OpenRouter | `qwen/qwen3-vl-32b-instruct` | Harness-ranked smoke + repo-tracked live RU proof + operator-reported current-view compare | `generic_full` | Structured current-view compare and live RU proof observed; default still not promoted | The simple `macro_finish_form` smoke was weak, but repo-tracked `TASK-163-07` closeout now includes one green live `reference-understanding` proof lane on the shared squirrel reference image, and one operator-reported `2026-04-07` `reference_compare_current_view(...)` squirrel run on `blender-ai-mcp-guided-docker-openrouter` returned a parseable structured payload with actionable mismatch lists. Keep this model off the default recommendation path until the same behavior is reproduced in richer harness-ranked staged loop coverage; the reported compare output still showed quality caveats such as a truncated `next_corrections` item. |
...
- `VISION_REFERENCE_CLASSIFIER_MAX_LABELS` (`1..5`, defaults to `5`)
