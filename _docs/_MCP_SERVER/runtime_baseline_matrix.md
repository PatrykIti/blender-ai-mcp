...
- Supported task-capable runtime pair: `fastmcp 3.1.1` + `pydocket 0.18.2`
- Supported FastMCP line for current platform work: `3.1.x`
...
|---|---|---|---|---|
| `3.11` | `fastmcp 3.1.1` + `pydocket 0.18.2` | Supported | Full current platform baseline including task mode | Explicit aligned pair validated during TASK-099 |
| `3.11` | `fastmcp 3.1.x` + `pydocket 0.18.x` | Expected compatible | Same as validated pair | Treat as candidate line, but keep validation centered on the locked pair |
| `3.12` | `fastmcp 3.1.x` + `pydocket 0.18.x` | Expected supported | Same as Python 3.11 baseline | Treat as compatible so long as repo tests stay green |
