...
| `step_status` | Coarse status such as `ready`, `blocked`, `needs_checkpoint`, or `needs_validation` |
...
- if the optional segmentation sidecar is enabled on runtime config, staged
  compare/iterate now try one bounded advisory-only packet-local sidecar pass;
  failures or empty sidecar results report
  `part_segmentation.status="unavailable"` instead of breaking the staged loop
...
- when emitted, `correction_candidates` and `truth_followup` stay ahead of
  perception hints; normal clean compact paths use `reference_orchestrator_feedback`,
  top-level `correction_focus`, `planner_summary`, and `action_hints` instead
...
Stage compare/iterate responses now also expose `guided_reference_readiness`, `reference_understanding_summary`, `reference_understanding_gate_ids`, `active_gate_plan`, top-level gate summaries (`gate_statuses`, `completion_blockers`, `next_gate_actions`, `recommended_bounded_tools`), `assembled_target_scope`, `budget_control`, `refinement_route`, `refinement_handoff`, `planner_summary`, `silhouette_analysis`, `action_hints`, and `part_segmentation`, so assembled-model correction flows can consume explicit session readiness, the current pre-build reference-understanding contract/linkage, the normalized gate plan and immediate gate repair path, a structured target scope, explicit trimming metadata, a deterministic refinement-family decision, compact planner provenance/blockers, typed perception metrics/tool hints, and a bounded advisory-only sidecar envelope instead of inferring everything from loose `target_object` / `target_objects` / `collection_name` fields. Heavy truth/planner detail (`truth_bundle`, `truth_followup`, `correction_candidates`, and rich `planner_detail`) is emitted on `preset_profile="rich"`, error paths, hard-failure paths, or compact uncertainty paths; normal clean compact runs keep those details summarized through `reference_orchestrator_feedback`, `correction_focus`, `budget_control`, and additive packet diagnostics when needed. When the run uses multiple compare packets, packet synthesis, model-aware budget pressure, rich delivery, or packet uncertainty, the same staged responses can now also surface additive `compare_diagnostics` with packet ids, packet-local view/scope slices, packet extraction/ranking status, packet uncertainty/budget notes, and typed packet-local `support_evidence` derived from the silhouette/action-hint seam plus optional compare-time segmentation support. Candidate-level `vision_evidence.packet_evidence_refs` carries bounded packet ids/statuses for join-back to `compare_diagnostics`, while full packet provenance remains on the diagnostics surface. Super-complex 6-12 image runs resolve the effective `VISION_MAX_IMAGES` limit before packet execution; large same-view reference sets become bounded reference slices, and mixed clean/corrective packet conclusions surface as conflict uncertainty instead of silently strengthening synthesis. Packet compare now runs extraction first and ranking second only when extraction says the packet is worth ranking; clean, low-information, and blocked packets skip the ranking pass explicitly, and ranking failure leaves the extraction evidence intact while marking the packet `ranking_status="error"` even on the compact path. The staged compare input-character limit is now runtime-owned through `VISION_MAX_INPUT_CHARS`, and `budget_control.max_input_chars` reflects that same resolved limit instead of a compare-only constant. `budget_control` also carries configured and effective image/input/output limits plus fail-safe clipping fields, so operators can distinguish requested runtime intent from the bounded values actually used by the runner and staged compare. OpenRouter-backed vision requests now attempt one bounded lazy `/models` catalog lookup and prefer live model capability metadata over reviewed fallback profiles when the requested model id is found. On `reference_iterate_stage_checkpoint(...)`, the loop-facing `correction_focus` now prefers ranked candidate summaries when they are present, unresolved `completion_blockers` can also move `loop_disposition` to `inspect_validate`, and high-priority deterministic truth findings still escalate the same way instead of waiting only for repeated vision focus. `planner_summary` is the first planner-facing read target: it reports the selected family, target scope, source-class provenance, typed blockers, and required support tools such as `scene_view_diagnostics(...)` when staged view evidence is missing before sculpt handoff. `action_hints` complement that loop by exposing deterministic silhouette-driven tool suggestions, while `part_segmentation` stays `disabled` by default and becomes packet-bounded advisory sidecar output only when an operator explicitly enables the separate sidecar path. Collection/object-set targeting now also avoids obviously accessory-first primary anchors when a more structural target is present, expands supported creature scopes into deterministic semantic scope clusters such as `Body + Head`, `Tail`, and `Ears`, keeps multiple failing required seams live together internally for feedback/candidate synthesis, normalizes vision-side `recommended_checks` to canonical MCP tool ids or drops them, applies model-aware budget control when the active runtime profile is too small for the full payload, and exposes `refinement_route` for bounded family choices such as `macro`, `modeling_mesh`, `sculpt_region`, or `inspect_only`. Sculpt stays hidden on the normal guided surface; `refinement_handoff` is recommendation-only, can be `ready`, `blocked`, or `suppressed`, and its bounded deterministic subset is `sculpt_deform_region`, `sculpt_inflate_region`, `sculpt_pinch_region`, and `sculpt_crease_region`.
...
budget/route/planner summaries, perception hints, and compact
`reference_orchestrator_feedback`, but omit the full capture list and heavy
truth/candidate/planner-detail blobs on normal clean compact paths. Rich/debug
capture and truth detail remains available through the richer profile path, and
compact uncertainty, error, or hard-failure paths may still emit additive
diagnostics.
During active assembled-workset checkpointing, the
...
top-level iterate fields keep `correction_focus`,
`reference_orchestrator_feedback`, hint, budget, and route summaries, while
duplicated nested `truth_bundle`, `truth_followup`, full
...
| `reference_compare_checkpoint` | `checkpoint_path` (str), `checkpoint_label` (str, optional), `target_object` (str, optional), `target_view` (str, optional), `goal_override` (str, optional), `prompt_hint` (str, optional) | Compares one current checkpoint image against the active goal plus attached reference images and returns bounded vision interpretation for the next correction step. |
| `reference_compare_current_view` | `checkpoint_label` (str, optional), `target_object` (str, optional), `target_view` (str, optional), `goal_override` (str, optional), `prompt_hint` (str, optional), viewport/camera args | Captures one current viewport/camera checkpoint using the bounded `scene_get_viewport` semantics, then compares it against the active goal plus attached reference images. |
| `reference_compare_stage_checkpoint` | `target_object` (str, optional), `target_objects` (array, optional), `collection_name` (str, optional), `checkpoint_label` (str, optional), `target_view` (str, optional), `goal_override` (str, optional), `prompt_hint` (str, optional), `preset_profile` (`compact`/`rich`) | Captures one deterministic multi-view stage checkpoint for a target object, object set, collection, or full assembled scene, then compares that view-set against the active goal plus attached reference images. May expose additive `compare_diagnostics`, packet-local `support_evidence`, `budget_control` detail, `reference_orchestrator_feedback`, and architecture packet scope labels such as `Facade + Openings`, `Roofline`, and `Supports` on rich delivery, multi-packet synthesis, model-budget pressure, or packet-uncertainty paths; normal clean compact paths omit heavy truth/candidate detail. |
| `reference_iterate_stage_checkpoint` | `target_object` (str, optional), `target_objects` (array, optional), `collection_name` (str, optional), `checkpoint_label` (str, optional), `target_view` (str, optional), `goal_override` (str, optional), `prompt_hint` (str, optional), `preset_profile` (`compact`/`rich`) | Runs one session-aware checkpoint iteration: capture deterministic stage views, compare them to the references, remember the previous correction focus, and return whether to continue building, inspect/validate, or stop. Top-level `compare_diagnostics` remains available for rich delivery, multi-packet synthesis, model-budget pressure, or packet uncertainty even when compact nested debug detail is omitted, and `reference_orchestrator_feedback` remains the compact next-step owner seam. |
...
| `router_get_status` | *none* | Returns current router session state, visibility diagnostics, pending clarification info, active `guided_handoff` when present, `guided_flow_state`, `guided_reference_readiness`, `reference_orchestrator_feedback`, and router/component stats. |
