...
| `macro_adjust_tail_arc` | `segment_objects`, `rotation_axis`, `total_angle`, `direction`, `segment_spacing`, `apply_rotation` | Bounded macro for adjusting an ordered tail-segment chain into a planar arc through deterministic per-segment placement and optional progressive rotation. | ✅ Done |
