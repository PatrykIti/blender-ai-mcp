...
| `scene_relation_graph` | `target_object` (optional), `target_objects` (optional), `collection_name` (optional), `goal_hint` (optional) | Returns one compact read-only relation graph derived from the current measure/assert truth layer, including bounded attachment/support/symmetry interpretations and additive world-frame `direction_world` metadata when justified. On `llm-guided`, the same authoritative payload can update `active_gate_plan` statuses, evidence refs, completion blockers, stale markers, and bounded repair-tool hints for relation-backed quality gates. | ✅ Done |
