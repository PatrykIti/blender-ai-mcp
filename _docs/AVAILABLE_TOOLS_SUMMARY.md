...
| `macro_adjust_head_body_proportion` | `head_object`, `body_object`, `expected_ratio`, `head_axis`, `body_axis`, `scale_target`, `tolerance`, `uniform_scale`, `max_scale_delta` | Bounded macro for repairing head/body proportion drift with an explicit target ratio and a bounded scale adjustment. | ✅ Done |
